---
tags: [engineering, hardware, diy, sustainability, manufacturing, electronics]
---

# Engineering & Hardware Projects

## Electronics & Home Automation
- [ ] Home automation: temperature, moisture, flow, hinge control
- [ ] Home network (LAN, fibre, HAM/wireless)
- [ ] Home control website / interface
- [ ] Solar power system (buy, test, install batteries)
- [ ] Auto watering system
- [ ] Raspberry Pi projects
- [ ] CNC machine — starting point for wall manufacturing
- [ ] Nixie tube displays
- [ ] Laminar flow water display (lights through laminar flow)
- [ ] Light displays
- [ ] Fix laptop with Linux
- [ ] WiFi light bulb / WiFi stick-on switch
- [ ] Develop different comms protocols
- [ ] GoProamera setups (get more)
- [ ] Electric cat feeder

## Vehicles & Transport
### Electric Vehicles to Design/Build
- Electric bike (solar-assisted for France trip concept)
- Electric motorcycle (naked / dirt / cafe racer)
- Micro cars
- Covered scooters
- Trucks/vans
- Kei trucks with roof rack
- Food trucks / shop trucks
- Post buggies
- Sport cars
- Trailers
- Car exterior upgrade panels

### Other Vehicle Projects
- Fix/upgrade existing motorcycle
- Connect phone to car: speed, engine diagnostics, etc.
- GPS-based non-tracking phones (VHF)
- CAN-AM vehicles (small)

## Manufacturing Ventures

### Wall Factory
- CNC machine for wall panels
- Prefab affordable housing construction
- Government contracts: state housing, schools, police/fire/ambulance stations
- Middle East / Russia export potential

### Full Manufacturing Stack
- Timber mills
- Sheet metal fabrication
- Glass window production
- Recycling facilities (metal, glass, timber, drywall)
- Copper wire production
- Plastic pipe production
- Solar panel factory
- Vehicle manufacturing
- Telecom — fibre optic

### Tools & Means of Production
- Company to make affordable, high-quality means of production for medium-scale expandable production
- CAD + CNC classes to teach others
- Store where people can make their own electronics (CNC/3D printer, casings)
- PC upgrade kits: CPU, RAM, HDD, OS
- Places to send and repair old tech / cables

## Sustainable Development

### Self-Sufficient Home Model
- Power: solar + battery storage (nickel-iron batteries)
- Water: rainwater collection, filtration, IBC tank system
- Food: garden, chickens, composting
- Telecom: local network / co-op
- Transport: electric bike, scooter

### Community-Scale Ideas
- Small-scale sustainable development
- Live-work areas to reduce travel distance
- Walk/bike/e-scooter self-sufficient zones
- Incorporate own city governance
- Reduce council service dependency

## Energy Projects
- Solar panels
- Wind turbine
- Micro-hydro
- Nickel-iron battery storage
- Kilowatt/production-capacity coin concept
- Harvestable ocean CO₂-absorbing plants
- Vacuum + magnet rails (maglev concept)

## Science & Research Interests
- Hydraulic physics
- Optimising spectrum / utilising other frequency bands
- Automated excavation equipment
- Micro-apartment designs

## Specific Build List
- [ ] Garage door automation
- [ ] Buy good soldering iron
- [ ] Buy microphone
- [ ] Buy air compressor
- [ ] Rusty parts tumbler (from sander)
- [ ] Turn-signal helmet
- [ ] Korean table
- [ ] Router table
- [ ] Maker space setup

## Manufacturing Facility Visits (Research)
- Electronics factory
- GJ Housing
- Toyota (high-tech automation)
- Ramset
- Timber (Rotorua)
- Gib (Gib Co, Auckland)
- Recycling (Auckland)
- Glass factory
- Windows/doors factory
- Brick/block production
- Concrete plant

---

## Workshop Equipment Wishlist
- [ ] Fine soldering iron
- [ ] Project holder
- [ ] Microscope / computer microscope
- [ ] Exhaust fan (for fumes)
- [ ] Small drill press
- [ ] Small lathe
- [ ] Variable power supply
- [ ] Oscilloscope
- [ ] 2-axis CNC machine
- [ ] Actuated armature for nail gun

## Home Automation (Specific)
- Access card machine
- GSM board
- Networked garage door (upgradeable site interface)
- Wireless light socket / switch
- Small solar system (expandable)

## Means of Production Equipment — Detail
Full list for a community production facility:
- Soap presses
- Toothpaste mixers
- Makeup moulds
- Chemical testers
- Kitchen equipment
- Industrial bottling machines (customers come and fill their own bottles)
- Electronic repair equipment
- Industrial sewing equipment
- Shoe sole moulds
- Car repair + remodeling (interior/exterior, parts recycling)
- CNC machines
- Jewellery manufacturing equipment
- Multi-printing equipment
- Machining tools to make one-off moulds for presses
- Open-source brand names with quality control

## Can-Am Trailer Types
Modular workshop trailers for mobile production:
- Woodwork trailer
- Metalwork trailer
- Electrical trailer
- Food trailer (with appliances)
- Specialty medical trailer

## Dev Co — Housing Vision
House 1 million people:
- Energy and water self-sufficient per development
- Each local area has a cache server
- Each home has a small server
- Internet self-sufficient (distributed)
- Provide model for sustainable design
- Wall manufacturing: inputs for other companies to make affordable sustainable housing
- Build efficient appliances to make solar more accessible
- Create infrastructure for decentralised internet
- Build mass transit systems
- Build tools and means of production equipment
- Provide employment in poor areas
